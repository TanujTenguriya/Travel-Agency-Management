  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-400 to-purple-600 p-6 flex flex-col items-center">
      <motion.div 
        ...
        <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">🏨 Find Your Perfect Stay</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" placeholder="Enter city or hotel name" value={location} onChange={(e) => setLocation(e.target.value)} className="p-2 border rounded w-full" />
          <input type="date" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} className="p-2 border rounded w-full" />
          <input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} className="p-2 border rounded w-full" />
          <div className="flex space-x-4">
            <input type="number" min="1" value={guests} onChange={(e) => setGuests(e.target.value)} className="p-2 border rounded w-24" placeholder="Guests" />
            <input type="number" min="1" value={rooms} onChange={(e) => setRooms(e.target.value)} className="p-2 border rounded w-24" placeholder="Rooms" />
          </div>
        </div>
        <motion.button 
            ...
            {filteredHotels.map((hotel) => (
              <motion.div 
                ...
                <div className="relative">
                  {hotel.images?.length ? (
                    <img src={mainImages[hotel._id]} alt="Hotel" className="w-full h-48 object-cover rounded" />
                  ) : (
                    <div className="h-48 bg-gray-300 flex items-center justify-center text-gray-500">No image</div>
                  )}
                  <div className="absolute top-2 right-2 bg-white px-2 py-1 rounded shadow-md text-gray-800 text-sm">⭐ {hotel.rating} / 5</div>
                </div>
                <div className="p-4">
                  <h3 className="text-xl font-bold">{hotel.hotelName}</h3>
                  <p className="text-gray-600">{hotel.location}</p>
                  <p className="text-green-600 font-bold">₹{hotel.pricePerNight} / night</p>
                  <motion.button 
                    whileHover={{ scale: 1.1 }} 
                    whileTap={{ scale: 0.9 }}
                    className="mt-4 w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700">
                    <Link to="/payment" state={{ amount: hotel.pricePerNight }}>Book Now</Link>
                  </motion.button>
                </div>
              </motion.div>
            ))}
